...
int main() {
    ...
    md << "$$\nR_T = " << formatDouble(RT) << "\\Omega \\quad \\text{(Computed)}\n$$\n\n";
    ...
    md << "$$\nR_{T\\text{min}} = " << formatDouble(RT_min) << "\\Omega \\quad \\text{(Computed)}\n$$\n\n";
    ...
    md << "$$\nR_{T\\text{max}} = " << formatDouble(RT_max) << "\\Omega \\quad \\text{(Computed)}\n$$\n\n";
    ...
    ofstream mdFile("parallel_circuit.md");
    if (!mdFile) {
        cerr << "Error: Could not create 'parallel_circuit.md'" << endl;
        return 1;
    }
    mdFile << md.str();
    mdFile.close();
    
    cout << "Markdown file 'parallel_circuit.md' has been generated successfully." << endl;
